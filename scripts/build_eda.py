...
## 구성
0. 환경 → 1. 데이터 개요(자체 촬영 25장) → 2. OCR 인식 현황(PP-OCRv5) →
...
`run_ocr.py`로 25장을 인식한 결과(`ocr_summary.csv`, `ocr_lines.csv`).
...
1. **데이터 다양성 확보** — 25장이 업종·난이도(저해상·초고밀도·다단·세로쓰기·손글씨)를 폭넓게 커버 → 오류사례가 풍부.
2. **v5 인식 신뢰도는 높음(평균 0.92, 저신뢰 3.4%)** — 단, 신뢰도가 높아도 **밀도/레이아웃 때문에 '메뉴 복원'은 별개**(5.png).
3. **개선의 지렛대 = 모델 선택(v3→v5)** — 복원율 65%→84%(25장). 특히 저해상·초고밀도·손글씨에서 결정적. 단 일부 세로쓰기(12번)에선 v5 하락(정직한 한계). (파인튜닝 아님 → `WORKLOG.md` 6-2)
